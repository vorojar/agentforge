...
describe("SQLiteAdapter", () => {
  ...
  describe("Skills", () => {
    ...
    it("should get a skill by id", () => {
      const created = db.createSkill({ name: "s1", content: "c1" });
      const fetched = db.getSkill(created.id);
      expect(fetched).toEqual(created);
    });

    it("should return null for non-existent skill", () => {
      expect(db.getSkill("non-existent")).toBeNull();
    });

    it("should get a skill by name", () => {
      db.createSkill({ name: "unique-skill", content: "content" });
      const fetched = db.getSkillByName("unique-skill");
      expect(fetched).not.toBeNull();
      expect(fetched!.name).toBe("unique-skill");
    });

    it("should return null for non-existent skill name", () => {
      expect(db.getSkillByName("no-such-skill")).toBeNull();
    });

    it("should list skills", () => {
      db.createSkill({ name: "s1", content: "c1" });
      db.createSkill({ name: "s2", content: "c2" });
      expect(db.listSkills()).toHaveLength(2);
    });

    it("should update a skill", () => {
      const skill = db.createSkill({ name: "old", content: "old content" });
      const updated = db.updateSkill(skill.id, { name: "new", enabled: false });
      expect(updated!.name).toBe("new");
    ...
    it("should return null when updating non-existent skill", () => {
      expect(db.updateSkill("non-existent", { name: "X" })).toBeNull();
    });

    it("should delete a skill", () => {
      const skill = db.createSkill({ name: "delete-me", content: "c" });
      expect(db.deleteSkill(skill.id)).toBe(true);
      expect(db.getSkill(skill.id)).toBeNull();
    });

    it("should return false when deleting non-existent skill", () => {
      expect(db.deleteSkill("non-existent")).toBe(false);
    });
  });
